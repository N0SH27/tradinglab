...
export const CONTEXT_HISTORY: readonly ContextHistory[] = [
  {
    id: 'ctx-20260825',
    date: '2026.08.25',
    reason: 'C1 迁移：既有生产状态登记为初始版本（15 节点语义状态 + MAP_ERA + 5 条 Observation + CYCLE_INDUSTRIES stage）。此为 versioned system 的第一个可观察状态，此前演化历史已永久缺失，不倒填。（2026.09.06 re-baseline：经 HSN 裁决，initial 快照 now 层由 5 条补足为当前全量 39 条 Observation，满足"initial 快照完整"断言；now 层的历史演化不由 initial 承担，而由其后 Version 序列与最新滚动条目承担——本次为基线维护操作，非历史倒填。）（2026.09.13 re-baseline：36 周消化批 +8 条 Observation，initial 快照 now 层同步补足为当前全量 47 条，同为基线维护操作。）',
    snapshot: {
      map: {
        era: {
          current: 1.8,
      ...
      now: {
        observations:         [
          ...
          },
          {
            ...
            "summary": "表外担保约 5300 亿美元（对比表内负债约 910 亿），累计 AI CapEx 约 11 万亿美元（CY24–29），兜底约 6.5GW（< 全球 3%）——\"正面我赢、反面谁输\"的循环融资结构首次有了量化轮廓。当前直接敞口有限，但作为 AI CapEx 的结构性风险点立档值守：验证锚 = 后续季报表外担保余额（L4-C10 候选，2026.09.13 本人裁决通过——规律立、参数不锁）。（2026.09）",
            "date": "2026.09",
            "mapNodeId": "gpu"
          },
          {
  ...
  },
  {
    id: 'ctx-20260913',
    date: '2026.09.13',
    reason: '36 周消化批（2026-09-13 入库）：NOW 整表滚动入账——新增 fed-rate-path-reversal / supply-shock-inflation-oil / memory-profit-extreme-divergence / fcc-optical-event-cleared / robot-production-slope / nvidia-backstop-structure / compute-fifteen-five-plan / gold-4400-reserve-shift 共 8 条；now.observations 为整表替换语义，本条使 fold 与生产当前态追平。Map 语义六字段、MAP_ERA、CYCLE 本周不变，不入账。',
    changes: {
      now: {
        observations:         [
          {
  ...
  },
  {
    id: 'ctx-20260913-2',
    date: '2026.09.13',
    reason: '同日第二条：36 周 7 项裁决采纳落账——nvidia-backstop-structure 条目「L4-C10 候选，待本人裁决」→「2026.09.13 本人裁决通过——规律立、参数不锁」；now.observations 为整表替换语义，本条使 fold 与生产当前态追平。Map 语义六字段、MAP_ERA、CYCLE 不变，不入账。',
    changes: {
      now: {
        observations:         [
          {
